...
import { findByPage } from '../services/manage-by-frequency-limit-controller';

export default {
  namespace: 'FrequencyLimitModel',
  ...
  effects: {
    *findByPage({ payload }, { select, call, put }) {
      let queryParam = yield select(state => state.FrequencyLimitModel.queryParam);
      let pagination = yield select(state => state.FrequencyLimitModel.pagination);
